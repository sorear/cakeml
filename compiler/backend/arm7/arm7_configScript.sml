...
val arm7_data_conf = ``<| tag_bits:=0; len_bits:=0; pad_bits:=1; len_size:=20; has_div:=F; has_longdiv:=F; has_fp_ops:=T; has_fp_tern:=T; call_empty_ffi:=F; gc_kind:=Simple; big_endian:=F; strings:=[] |>``
val arm7_word_conf = ``<| bitmaps := []:32 word list; stack_frame_size := LN; static_offset:=0w |>``
