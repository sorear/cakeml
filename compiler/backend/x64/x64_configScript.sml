...
val x64_data_conf = ``<| tag_bits:=4; len_bits:=4; pad_bits:=2; len_size:=32; has_div:=F; has_longdiv:=T; has_fp_ops:=T; has_fp_tern:=F; call_empty_ffi:=F; gc_kind:=Simple; big_endian:=F; strings:=[] |>``
val x64_word_conf = ``<| bitmaps := []:64 word list; stack_frame_size := LN; static_offset := 0w |>``
