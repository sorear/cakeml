...
val mips_data_conf = ``<| tag_bits:=4; len_bits:=4; pad_bits:=2; len_size:=32; has_div:=T; has_longdiv:=F; has_fp_ops:=F; has_fp_tern := F; call_empty_ffi:=F; gc_kind:=Simple; big_endian:=F; strings:=[] |>``
val mips_word_conf = ``<| bitmaps := []:64 word list; stack_frame_size := LN; static_offset := 0w |>``
