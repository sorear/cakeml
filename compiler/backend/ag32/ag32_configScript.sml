...
val ag32_data_conf = ``<| tag_bits:=0; len_bits:=0; pad_bits:=1; len_size:=20; has_div:=F; has_longdiv:=F;
  has_fp_ops:=F; has_fp_tern:=F; call_empty_ffi:=F; gc_kind:=Simple; big_endian:=F; strings:=[] |>``
